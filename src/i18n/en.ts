export const en = {
  meta: {
    title: "CITOV — There is another way",
    description: "Smart placement through conversation — without search",
  },
  app: {
    name: "CITOV",
    versionLabel: "Version {version}",
  },
  language: {
    switch: "Language",
    english: "English",
    hebrew: "עברית",
  },
  home: {
    tagline: "There is another way",
    description:
      "A natural conversation with a smart placement agent. It understands you deeply, matches with precision, and shows you only the jobs that approved you.",
    employerDescription:
      "A natural conversation with a smart hiring agent. It understands the role and your company, and surfaces candidates who truly fit.",
    googleSignIn: "Sign in with Google",
    connectedAs: "Signed in as {name}",
    continueAsEmployee: "Continue as job seeker",
    continueAsEmployer: "Continue as employer",
    signOut: "Sign out",
    googleSetupHint:
      "To enable: add `AUTH_GOOGLE_ID`, `AUTH_GOOGLE_SECRET`, and `AUTH_SECRET` to `.env.local` (see README). Demo mode is available below for now.",
    demoEmployee: "Employee demo",
    demoEmployeeHint: "No Google — quick testing",
    demoEmployer: "Employer demo",
    demoEmployerHint: "No Google — quick testing",
    defaultEmployeeName: "New candidate",
    defaultEmployerName: "New employer",
    realUsersTitle: "Sign in for real users",
    iAmEmployee: "I am looking for work",
    iAmEmployer: "I am hiring",
    googleNotConfigured: "Google Auth is not configured on the server yet.",
    afterSignInHint: "After signing in, continue as a candidate.",
    demoEmployeeDev: "Candidate demo (dev only)",
    demoEmployerDev: "Employer demo (dev only)",
    adminPortal: "Admin portal",
    openAuthHint: "Google sign-in is paused — start chatting as a candidate.",
    employerOpenAuthHint: "Google sign-in is paused — start chatting as an employer.",
    employerAfterSignInHint: "After signing in, continue as an employer.",
    openingRole: "Opening your screen…",
    realUsersTitleOpen: "Choose how you want to continue",
    devSignIn: "Test sign-in (no Google)",
    candidateGoogleHint: "Sign in as a candidate with Google.",
    candidateGoogleButton: "Enter and find the job for you",
    employerGoogleHint: "Sign in as an employer with Google.",
    employerGoogleButton: "Enter and find the right candidate",
    heroTitleCandidate: "Find work through conversation — not endless search",
    heroTitleEmployer: "Hire through conversation — not endless screening",
    pillarChatTitle: "Natural chat",
    pillarChatBody: "Share your story in a human dialogue — no rigid forms.",
    pillarMatchTitle: "Smart matching",
    pillarMatchBody: "AI understands context and surfaces truly relevant fits.",
    pillarApprovedTitle: "Approved only",
    pillarApprovedBody: "You see opportunities that were approved for you — no noise.",
    ctaCardTitle: "Get started",
    ctaCardHint: "Sign in and start your first conversation in under a minute.",
    switchToEmployer: "Hiring? Go to the employer entry",
    switchToCandidate: "Job seeking? Go to the candidate entry",
  },
  devLogin: {
    title: "Development sign-in",
    hint: "Pick how to enter — no Google required for internal testing.",
    admin: "Admin",
    existing: "Existing user",
    newUser: "New user",
    pickUser: "Choose user",
    nameOptional: "Display name (optional)",
    namePlaceholder: "e.g. Test candidate",
    enter: "Enter",
    roleEmployee: "candidate",
    roleEmployer: "employer",
  },
  session: {
    noActiveSession: "No active session.",
    backToStart: "Back to start",
    reconnect: "Sign in again",
    loading: "Loading…",
  },
  settings: {
    title: "Settings",
    language: "Language",
    privacy: "Privacy policy",
    terms: "Terms of service",
    about: "About",
    report: "Report a problem",
    rate: "Rate the app",
    rateTitle: "How was your experience?",
    rateHint: "Your rating helps us improve CITOV.",
    cancel: "Cancel",
    sendRating: "Send",
    reportSubject: "CITOV — problem report",
    reportBody: "Describe the problem:",
    rateSubject: "CITOV — app rating",
    rateBody: "My rating",
    defaultRole: "Default role",
    roleEmployee: "Job seeker",
    roleEmployer: "Employer",
    roleClear: "Ask each time on home",
    adminPortal: "Admin portal",
    signOut: "Sign out",
  },
  legal: {
    privacyTitle: "Privacy policy",
    termsTitle: "Terms of service",
    aboutTitle: "About CITOV",
    back: "Back",
    privacyBody:
      "CITOV stores the profile and chat information you share in order to match candidates and employers. We use Google sign-in for authentication. We do not sell your personal data. Contact us to request deletion of your account data.",
    termsBody:
      "CITOV is a placement matching service. Matches are recommendations, not hiring guarantees. Users are responsible for the accuracy of information they provide. Misuse, harassment, or fraudulent profiles may lead to account removal.",
    aboutBody:
      "CITOV helps people find work through natural conversation with an AI placement agent — without traditional search. There is another way.",
  },
  employee: {
    subtitle: "Upload CV · Smart chat · Live profile",
    localMode: " · Local mode (no Gemini key)",
    chatTab: "Submit application",
    jobsTab: "Scheduled interviews ({count})",
    chatPlaceholder: "e.g. I'm looking for a sales role — flexibility on hours matters…",
    flowTitle: "Job search flow",
    stepCv: "Upload CV",
    stepChat: "Chat with agent",
    stepProfile: "Scheduled interviews",
    cvStepHint: "First step — upload your CV to unlock chat",
    profileStepHint: "Updates automatically from CV and conversation",
    applicationProgress: "Match level",
    applicationProgressLine: "{percent}% match level",
    applicationProgressHint: "Upload your CV and answer questions in the chat to increase your chances of being accepted for a position",
    cvUploadedMessage: "CV: {fileName}",
    chatHeroTitle: "Smart conversation with the agent",
    chatHeroLead: "After uploading your CV, share what you're looking for — the agent asks focused questions to refine your profile.",
    chatLockedTitle: "Chat unlocks after CV upload",
    chatLockedBody: "Upload your CV to start chatting. The agent reads it and asks targeted follow-up questions.",
  },
  employer: {
    subtitle: "Chat with the agent · Ranked candidates",
    localMode: " · Local mode (no Gemini key)",
    chatTab: "Chat",
    candidatesTab: "Candidates ({count})",
    chatPlaceholder: "e.g. Looking for a waiter for a restaurant in Tel Aviv…",
    jobsFilter: "Jobs",
    newJob: "New job",
    jobFallback: "Job {n}",
  },
  chat: {
    title: "Chat with the agent",
    subtitle: "A live conversation — at your pace",
    localMode: "Local mode",
    degradedMode: "Limited (Gemini load)",
    employeeEmptyHint:
      "Tell me about yourself freely — what you’re looking for, what matters, and what a good workday looks like.",
    employerEmptyHint:
      "Tell me about the role freely — who would truly fit your team, and what matters day to day.",
    typing: "Agent is typing…",
    send: "Send",
    replyFailed: "Could not get a reply",
    reset: "Reset chat",
    resetConfirm: "Clear this conversation?",
    attachCv: "Update CV",
    cvRequired: "Upload your CV to start chatting with the agent.",
    cvRequiredEmpty: "Upload your CV in the sidebar to unlock the chat and start your conversation.",
    cvRequiredPlaceholder: "Upload your CV first…",
  },
  fileImport: {
    choose: "Upload a file",
    uploadCv: "Upload CV",
    changeCv: "Change CV",
    expandCv: "Change CV",
    collapseCv: "Close",
    cvCaptured: "CV captured",
    saving: "Saving…",
    analyzing: "Analyzing…",
    analyzingProgress: "Analyzing… {percent}%",
    saved: "CV saved successfully",
    analyzed: "Analysis complete — profile updated",
    processing: "Reading…",
    done: "Added to your card",
    error: "Could not read the file",
    cvTitle: "Upload CV",
    cvHint: "PDF, DOCX or TXT — deep analysis fills your profile.",
    cvSummary: "CV received · ~{fields} details updated · {roles} roles in history",
    cvConflictsHint: "We'll clarify a few points in chat.",
    jobTitle: "Upload job description",
    jobHint: "PDF, DOCX or TXT — the agent reads it and fills the job card.",
  },
  profile: {
    yourCard: "Your profile",
    yourCardWithPercent: "Your profile {percent}%",
    hideCard: "Hide",
    showDetails: "Details",
    hideDetails: "Hide",
    jobCard: "Job profile",
    fieldsFilled: "{filled}/{total} fields filled · the rest are defined and waiting to be completed",
    autoFillHint: "Updates quietly from the chat · includes free text",
    flexibility: "Flexibility",
    knowledge: "Relevant knowledge",
    knowledgePercent: "{percent}%",
    knowledgeHint: "0% = no relevant knowledge · 100% = you shared all relevant knowledge",
    pendingQuestions: "{count} field questions are waiting in the chat",
    emptyValue: "—",
    flexibilityValue: "{value}/10",
    miniCardHint: "A short preview of your profile — not every internal detail.",
    miniCardEmpty: "Not enough details yet — continue in chat after uploading your CV.",
  },
  candidates: {
    empty:
      "No ranked candidates yet. Fill in the job card in chat — the system will bring matches.",
    roleNotSpecified: "Role not specified",
    personality: "Personality: {value}",
    flexibility: "Flexibility {value}/10",
    fit: "Good fit",
    notFit: "Not a fit",
    askPlaceholder: "Field question (shown to all candidates in this field)",
    ask: "Ask",
    defaultName: "Candidate",
    viewCv: "View CV",
  },
  jobs: {
    empty:
      "Only jobs where an employer has marked you as a potential candidate will appear here.",
    defaultTitle: "Job",
    approved: "Approved",
    salary: "Salary: {value}",
    interviewSlots: "Interview availability: {value}",
    defaultEmployerName: "Employer",
  },
  api: {
    missingDetails: "Missing required details",
    missingUserId: "Missing userId",
    internalError: "Something went wrong on the server. Please try again in a moment.",
    googleRequired: "Google sign-in required",
    demoDisabled: "Demo mode is disabled. Sign in with Google.",
    userNotFound: "User not found",
    unauthorized: "Not authorized for this user",
    questionTooShort: "Question is too short",
    defineFieldFirst: "Define the job field before asking field-wide questions",
  },
  cardFields: {
    candidate: {
      desiredRole: "Desired role",
      field: "Field",
      subField: "Sub-field",
      location: "Primary location",
      preferredCities: "Preferred cities",
      remotePreference: "Remote/hybrid preference",
      relocationWillingness: "Willing to relocate",
      experienceYears: "Years of experience",
      experienceLevel: "Experience level",
      skills: "Professional skills",
      softSkills: "Soft skills",
      languages: "Languages",
      hebrewLevel: "Hebrew level",
      englishLevel: "English level",
      education: "Education",
      certifications: "Certifications",
      licenses: "Licenses",
      computerSkills: "Computer skills",
      salaryExpectation: "Salary expectation",
      salaryMin: "Minimum salary",
      salaryMax: "Maximum salary",
      employmentType: "Employment type",
      hoursPerWeek: "Hours per week",
      shiftPreference: "Shift preference",
      nightShiftsOk: "Night shifts",
      weekendsOk: "Weekends",
      transportation: "Transportation",
      hasVehicle: "Private vehicle",
      drivingLicense: "Driving license",
      personality: "Personality",
      workStyle: "Work style",
      strengths: "Strengths",
      weaknesses: "Areas to improve",
      careerGoals: "Career goals",
      motivation: "Motivation",
      dealBreakers: "Deal breakers",
      noticePeriod: "Notice period",
      startDate: "Possible start date",
      availability: "General availability",
      interviewAvailability: "Interview availability",
      customerFacingOk: "Customer-facing work",
      physicalRequirementsOk: "Physical requirements",
      managementExperience: "Management experience",
      teamSizeManaged: "Team size managed",
      industryExperience: "Industry experience",
      portfolioUrl: "Portfolio / link",
      linkedinUrl: "LinkedIn",
      referencesAvailable: "References",
      narrative: "Free text / story",
      flexibility: "Match flexibility (1–10)",
      summary: "Summary",
    },
    job: {
      title: "Job title",
      field: "Field",
      subField: "Sub-field",
      location: "Location",
      workModel: "Work model (office/hybrid/remote)",
      scope: "Job scope",
      employmentType: "Employment type",
      hoursPerWeek: "Hours per week",
      shiftPattern: "Shift pattern",
      nightShiftsRequired: "Night shifts required",
      weekendsRequired: "Weekends required",
      salaryRange: "Salary range",
      salaryMin: "Minimum salary",
      salaryMax: "Maximum salary",
      benefits: "Benefits",
      mustHaves: "Must-haves",
      niceToHaves: "Nice to have",
      requiredLanguages: "Required languages",
      requiredLicenses: "Required licenses",
      requiredEducation: "Required education",
      minExperienceYears: "Minimum years of experience",
      seniorityLevel: "Seniority level",
      toolsRequired: "Tools/systems",
      personalityFit: "Personality fit",
      teamCulture: "Team culture",
      managementStyle: "Management style",
      teamSize: "Team size",
      reportsTo: "Reports to",
      customerFacing: "Customer-facing",
      physicalDemands: "Physical demands",
      dressCode: "Dress code",
      transportNeeded: "Transport/vehicle requirement",
      startDate: "Desired start date",
      urgency: "Hiring urgency",
      interviewSlots: "Interview windows",
      interviewProcess: "Interview process",
      probationPeriod: "Probation period",
      growthPath: "Growth path",
      companyDescription: "Company/business description",
      companySize: "Organization size",
      industry: "Industry",
      dealBreakers: "Candidate deal breakers",
      successMetrics: "Success metrics for the role",
      onboardingNotes: "Onboarding notes",
      contactName: "Hiring contact",
      contactPhone: "Contact phone",
      narrative: "Free text / story about the job",
      flexibility: "Requirement flexibility (1–10)",
      summary: "Job summary",
    },
  },
  flexibility: {
    title: "Job match flexibility level",
    decrease: "Decrease flexibility",
    increase: "Increase flexibility",
    veryFlexible: "1 · very flexible",
    exactOnly: "10 · exact only",
    tooltipExact: "1 — exact match only",
    tooltipFlexible: "10 — very flexible",
    hint: "Drag the slider or tap the bar to adjust.",
  },
  matching: {
    similarField: "Similar field ({candidate} ↔ {job})",
    roleMatch: "Role match",
    locationMatch: "Location match",
    skillsOverlap: "Skills overlap with requirements",
    flexibility: "Flexibility {value}/10",
    score: "Score {value}",
  },
} as const;


type DeepString<T> = {
  [K in keyof T]: T[K] extends string
    ? string
    : T[K] extends Record<string, unknown>
      ? DeepString<T[K]>
      : T[K];
};

export type Messages = DeepString<typeof en>;
